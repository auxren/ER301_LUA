...
function WaveForm:init(args)
  ...
  self:setClassName("Looper.WaveForm")
  local width = args.width or 4 * ply
  local head = args.head or app.error("%s: head is missing from args.",self)
  local graphic
  graphic = app.Graphic(0,0,width,64)
  self.mainDisplay = app.RecordHeadDisplay(0,0,width,64)
  ...
  self.subDisplay = app.RecordHeadSubDisplay()
  ...
  self:setRecordHead(head)
end

function WaveForm:setRecordHead(head)
  self.mainDisplay:setRecordHead(head)
  self.head = head
end
