...
local sensitivity = Env.EncoderSensitivity.DurationControl
...
function TimeChooser:init(channelCount)
  ...
  self.rateBox:addItem(string.format("%dkHz",app.getSampleRate()/1000))
...
function TimeChooser:onActivated()
  local largest = app.heapLargest(memUnit)
...
function TimeChooser:updateMemoryEstimate()
  ...
  local f = (4 * app.getSampleRate()) / memUnit
...
function TimeChooser:encoder(change,shifted)
  self.durationControl:encoder(change,shifted,sensitivity)
  self:updateMemoryEstimate()
  return true
end
...
function TimeChooser:enterReleased(shifted)
  if shifted then return false end
  if not self:updateMemoryEstimate() then
    local dialog = Message.Main("Not enough contiguous memory","to create this buffer.")
    dialog:activate()
    return true
  end
  local secs = self.durationControl:getTotalSeconds()
